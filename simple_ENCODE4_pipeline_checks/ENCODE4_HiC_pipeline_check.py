...
def check_encode4_hic_pipeline(exp_acc):
    ...
    for analysis in analysisObj:
        ...
        if sorted(analysis['pipelines']) != ENCODE4_HIC_PIPELINES:
            skipped_analyses_count += 1
            continue
        if sorted(analysis['pipelines']) == ENCODE4_HIC_PIPELINES and analysis['accession'] != latest:
        ...
        if (
            ...
            if sorted(
                ...
                print(preferred_default_output_type)
...
def get_parser():
    parser = argparse.ArgumentParser(
        description='Script to check result of ENCODE4 HiC processing on '
        'the ENCODE portal'
    )
    parser.add_argument(
        'exp_accs',
...
def main():
    ...
    for exp_acc in args.exp_accs:
        bad_reason, serious_audits, archiveAnalyses = check_encode4_hic_pipeline(
            exp_acc.strip()
        )
